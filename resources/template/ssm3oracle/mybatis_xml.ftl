...
<mapper namespace="com.${model_package}.bean.${class_name}">
	...
	<sql id = "TABLE_SEQUENCE">${table_name}_seq.nextval</sql>
	...
	<sql id="entity_properties">
		<#list table_column as c>
		${r"#"}{${c.nameJ},jdbcType=${c.jdbcType}}<#if c_has_next>,</#if>
		</#list>
	</sql>
	
	<!-- 适用于主键自增类型 -->
	<insert id="save" parameterType="${class_name}">
	...
	<select id="page" resultMap="${class_name?uncap_first}ResultMap" parameterType="${class_name}Search">
		select *
			from (select opage.*, ROWNUM as rnum
			   from ( select <include refid="table_columns" />	
					from ${table_name} <include refid="page_where" /> ) opage
						where ROWNUM &lt; ${r"#"}{pageLastIndex})
		where rnum &gt; ${r"#"}{pageFirstIndex}	
	</select>
